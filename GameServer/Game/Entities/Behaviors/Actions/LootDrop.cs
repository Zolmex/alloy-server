...
public record LootDrop : BehaviorScript {
    ...
    public override void Start(ref EntityContext host) {
        host.World.EventSystem.Subscribe(host.Entity, HandleLoot);
    }

    public override void End(ref EntityContext host, ref RealmTime time) {
        host.World.EventSystem.Unsubscribe(host.Entity, HandleLoot);
    }

    private void HandleLoot(ref DeathEvent evt) {
        var entityView = new EntityContext(evt.World, evt.Entity);
        var playerDrops = new Dictionary<int, Queue<Item>>();
        // TODO: Reimplement loot
