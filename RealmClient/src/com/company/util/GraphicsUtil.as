...
public class GraphicsUtil {
    ...
    public static function drawCutEdgeRectGraphics(x:int, y:int, width:int, height:int, cutLen:int, cuts:Array, g:Graphics):void {
        if (cuts[0] != 0) {
            g.moveTo(x, y + cutLen);
            g.lineTo(x + cutLen, y);
        } else {
        ...
        if (cuts[1] != 0) {
            g.lineTo(x + width - cutLen, y);
            g.lineTo(x + width, y + cutLen);
        } else {
            g.lineTo(x + width, y);
        }
        if (cuts[2] != 0) {
            g.lineTo(x + width, y + height - cutLen);
            g.lineTo(x + width - cutLen, y + height);
        } else {
            g.lineTo(x + width, y + height);
        }
        if (cuts[3] != 0) {
            g.lineTo(x + cutLen, y + height);
            g.lineTo(x, y + height - cutLen);
        } else {
            g.lineTo(x, y + height);
        }
        if (cuts[0] != 0) {
