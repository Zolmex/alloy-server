   ...
   public class MaskedImageSet
   {
      ...
      public function addFromBitmapData(images:BitmapData, masks:BitmapData, width:int, height:int) : void
      {
         ...
         for(var i:int = 0; i < imagesSet.images_.length; i++)
         {
            ...
            if (masksSet != null){
               if (i >= masksSet.images_.length){
                  trace("NOT ENOUGH MASKS, i:", i, "len:", masksSet.images_.length);
               }
               else{
                  mask = masksSet.images_[i];
               }
            }
            this.images_.push(new MaskedImage(imagesSet.images_[i],mask));
         }
      }
