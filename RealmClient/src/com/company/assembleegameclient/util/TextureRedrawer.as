...
public class TextureRedrawer {
   ...
   public static function redraw(tex:BitmapData, size:int, padBottom:Boolean, glowColor:uint, useCache:Boolean = true, sMult:Number = 5, outlineSize:Number = 1.4):BitmapData {
      ...
      modTex = GlowRedrawer.outlineGlow(modTex, glowColor, outlineSize, useCache);
      if (useCache) {
   ...
   public static function clearCache():void {
      ...
      for (sizeKey in cache_) {
         dict = cache_[sizeKey];
         for (colorKey in dict) {
            dict[colorKey].dispose();
            delete dict[colorKey];
         }
         delete cache_[sizeKey];
      }
      cache_ = new Dictionary();

      var shadeKey:int;
      var texKey:BitmapData;
      for (shadeKey in faceCache_) {
         dict = faceCache_[shadeKey];
         for (texKey in dict) {
            dict[texKey].dispose();
            delete dict[texKey];
         }
         delete faceCache_[shadeKey];
      }
      faceCache_ = new Dictionary();

      for (texKey in nonTransparentCache_){
         nonTransparentCache_[texKey].dispose();
         delete nonTransparentCache_[texKey];
      }
      nonTransparentCache_ = new Dictionary();
   }
