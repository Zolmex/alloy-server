    ...
    public static function getTierTag(itemData:ItemData, size:int = 12):UILabel {
        ...
        if (!itemData.Consumable && !itemData.Treasure && !itemData.PermaPet) {
            ...
            if (itemData.ItemLevel != -1) {
                color = TooltipHelper.getSpecialityColor(itemData);
                text = "Lv " + String(itemData.ItemLevel);
            }
            else if (itemData.Tier != -1) {
                color = 16777215;
                text = "T" + String(itemData.Tier);
            } else if (itemData.Rarity != null) {
                color = TooltipHelper.getSpecialityColor(itemData);
                text = TooltipHelper.getRarityTag(itemData);
            }
            if (text == null) {
