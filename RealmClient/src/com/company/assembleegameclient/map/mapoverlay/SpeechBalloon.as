...
import kabam.rotmg.emote.EmoteGraphicHelper;
...
public class SpeechBalloon extends Sprite implements IMapOverlayElement {
    ...
    private static function makeText(text:String, bold:Boolean, color:uint):Sprite {
        var helper:EmoteGraphicHelper = new EmoteGraphicHelper();
        return helper.getChatBubbleText(text, bold, color);
    }
    ...
    public function SpeechBalloon(go:GameObject, text:String, background:uint, backgroundAlpha:Number, outline:uint, outlineAlpha:Number, textColor:uint, lifetime:int, bold:Boolean, hideable:Boolean) {
        ...
        this.text_ = makeText(text, bold, textColor);
        this.text_.y = this.text_.height;
        addChild(this.text_);
        var w:int = this.text_.width + 4;
        this.offset_.x = -w / 2;
