...
public class EquipmentTile extends InteractiveItemTile {
    ...
    private static const greyColorFilter:ColorMatrixFilter = new ColorMatrixFilter(MoreColorUtil.singleColorFilterMatrix(0x16161f));
    ...
    public function setType(type:int):void {
        ...
        if (bd != null) {
            var regionBd:BitmapData = TextureRedrawer.removeTransparentPixels(bd);
            this.backgroundDetail = new Bitmap(regionBd);
            this.backgroundDetail.scaleX = 4;
            this.backgroundDetail.scaleY = 4;
            this.backgroundDetail.filters = [greyColorFilter];
            this.backgroundDetail.x = (WIDTH - this.backgroundDetail.width) / 2;
            this.backgroundDetail.y = (HEIGHT - this.backgroundDetail.height) / 2;
    ...
    override public function setItem(item:ItemData):Boolean {
        if (this.cooldownTimer == null){
            ...
            addChild(this.cooldownTimer);
        }
        else {
            removeChild(this.cooldownTimer);
            addChild(this.cooldownTimer); // Moves it back to the top
        }
        ...
        if (itemSprite.itemData != null) {
            if (itemSprite.itemData.Usable) {
                if (itemSprite.itemData.MultiPhase) {
                    ...
                    if (itemSprite.itemData.Spell)
                        this.minManaUsage = itemSprite.itemData.Spell.MpCost;
                    else if (itemSprite.itemData.Quiver)
                        this.minManaUsage = itemSprite.itemData.Quiver.MpCost;
                    else
    ...
    public function updateDim(player:Player):void {
        itemSprite.setDim(player && player.mp < this.minManaUsage);
        this.cooldownTimer.update();
    }

    public function abilityUse():void {
        this.cooldownTimer.start(this.itemSprite.itemData);
    }
