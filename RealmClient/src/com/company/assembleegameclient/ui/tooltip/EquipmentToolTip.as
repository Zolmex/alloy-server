...
import com.company.assembleegameclient.itemData.ConditionEffectDesc;
import com.company.assembleegameclient.itemData.CustomToolTipData;
import com.company.assembleegameclient.itemData.GemstoneBoost;
import com.company.assembleegameclient.itemData.GemstoneDesc;
...
import com.company.assembleegameclient.ui.GemstoneSlot;
...
public class EquipmentToolTip extends ToolTip {
    ...
    private var rarityText:SimpleText;
    ...
    private var rarityColor:uint;

    private var invSlot:int;

    private var gemstoneUI:Boolean;
    private var gemstoneSlots:Vector.<GemstoneSlot>;
    ...
    public function EquipmentToolTip(itemData:ItemData, owner:Player = null) {
        ...
        this.invSlot = owner != null ? owner.equipment_.indexOf(itemData) : -1;
        this.usableBy = IsUsableBy(owner, itemData.SlotType);
        this.rarityColor = TooltipHelper.getSpecialityColor(this.itemData);
        var backColor:uint = this.usableBy ? 0x363636 : 6036765;
        var outlineColor:uint = this.usableBy ? MoreColorUtil.adjustBrightness(this.rarityColor, -0.15) : 10965039;
    ...
    private function drawIcon():void {
        var tex:BitmapData = this.itemData.getRedrawnTexture(70, 0, true, getTimer());
    ...
    private function drawDisplayName():void {
        var text:String = TooltipHelper.getRarityText(this.itemData);
        var color:uint = text != "Untiered" ? this.rarityColor : 0xB3B3B3;
        this.displayText = new SimpleText(17, color, false, WIDTH - (this.icon.width + 30));
        this.displayText.text = this.itemData.GetDisplayName();
    ...
    private function drawRarity():void {
        var text:String = TooltipHelper.getRarityText(this.itemData);
        this.rarityText = new SimpleText(14, this.rarityColor, false, 0, 0, false, Parameters.SPIEGEL);
        this.rarityText.text = text;
        this.rarityText.useTextDimensions();
        if (Parameters.data_.toolTipOutline)
            this.rarityText.filters = FilterUtil.getTextOutlineFilter();
        else
            this.rarityText.filters = FilterUtil.getTextShadowFilter();
        this.rarityText.x = this.displayText.x;
        this.rarityText.y = this.displayText.y + this.displayText.height - 4;
        addChild(this.rarityText);
    }
    ...
    private function drawDesc():void {
        this.descText = new SimpleText(14, 0xB3B3B3, false, WIDTH - 10, 0, false, Parameters.SPIEGEL);
    ...
    private function makeAttributes():void {
        ...
        this.makeStatBoosts();
        this.makeGemstoneBoosts();
        this.makeGlobalAttributes();
        this.makeGemstoneAttributes();
    }

    private function makeGemstoneAttributes():void {
        if (!this.itemData.Gemstone || this.itemData.Gemstone.Boosts.length < 1)
            return;

        this.attributes += "Boosts equipment:\n";
        var amountColor:String = TooltipHelper.NO_DIFF_COLOR;
        if (this.usableBy) {
            amountColor = TooltipHelper.BETTER_COLOR;
        }
        for each (var boost:GemstoneBoost in this.itemData.Gemstone.Boosts) {
            var displayStat:String = boost.DisplayStat || boost.Stat;
            var amount:Number = boost.Amount;
            if (this.usableBy) {
                amountColor = TooltipHelper.BETTER_COLOR;
            } else amountColor = TooltipHelper.NO_DIFF_COLOR;
            if (this.equipData && this.equipData.Gemstone.Boosts && this.equipData.Gemstone.Boosts.length > 0) {
                var boost2:GemstoneBoost = GetGemstoneBoost(this.equipData.Gemstone.Boosts, boost.Stat);
                if (boost2) {
                    var amount2:int = boost2.Amount;
            ...
            this.attributes += "  ";
            var space:String = boost.BoostType == "Static" ? " " : "% ";
            if (boost.BoostTarget == "Item" && ItemConstants.isDecimalProperty(boost.Stat)) {
                amount *= 100;
            }

            amount = Math.round(amount);
            this.attributes += TooltipHelper.wrapInFontTag(GetSign(amount) + amount + space, amountColor) + TooltipHelper.wrapInFontTag(displayStat, amountColor) + "\n";
        }
    }
    ...
    private function makeProjEffects(proj:ProjectileDesc, proj2:ProjectileDesc):void {
        ...
        for (var i:int = 0; i < proj.Effects.length; i++) {
            var condEff:ConditionEffectDesc = proj.Effects[i];
    ...
    private function makeProjRoF(proj:ProjectileDesc, proj2:ProjectileDesc):void {
        ...
        if (this.equipData) {
            ...
            color = TooltipHelper.BETTER_COLOR;
        this.attributes += "Rate of Fire: " + TooltipHelper.wrapInFontTag(nf.formatNumber(rof / 100), color) + "\n";
    }

    private function makeProjArcGap(proj:ProjectileDesc, proj2:ProjectileDesc):void {
        if (this.itemData.ArcGap <= 0 || this.itemData.NumProjectiles < 2)
            return;
    ...
    private function makeActivateEffects():void {
        ...
        for each (ae in effs) {
            ...
            var condDuration:Number = ae.CondEffectDuration;
            ...
            if (this.equipData && this.equipData.ActivateEffects && this.equipData.ActivateEffects.length > 0) {
                ...
                if (ae2) {
                    ...
                    condDuration2 = ae2.CondEffectDuration;
            ...
            switch (ae.EffectName) {
                ...
                case ActivationType.BULLET_NOVA:
                    this.attributes += TooltipHelper.wrapInFontTag("Spell: ", TooltipHelper.SPECIAL_COLOR) + TooltipHelper.wrapInFontTag(String(itemData.Spell.NumProjectiles), TooltipHelper.NO_DIFF_COLOR) + " shots";
                    this.attributes += "\n-MP Cost: " + TooltipHelper.wrapInFontTag(String(itemData.Spell.MpCost), TooltipHelper.NO_DIFF_COLOR);
                    break;
    ...
    private function makeGemstoneBoosts():void {
        if (!this.itemData.Gemstones || this.itemData.Gemstones.length < 1) {
            return;
        }

        this.attributes += "Gemstone Boosts:\n";
        var statBoosts:Dictionary = new Dictionary();
        for each (var gemType:int in this.itemData.Gemstones) {
            var gem:ItemData = new ItemData(gemType);
            if (!gem.Gemstone) {
                continue;
            }

            for each (var boost:GemstoneBoost in gem.Gemstone.Boosts) {
                if (statBoosts.hasOwnProperty(boost.Stat)) {
                    var val:GemstoneBoost = statBoosts[boost.Stat];
                    val.Amount += boost.Amount;
                    continue;
                }

                statBoosts[boost.Stat] = boost;
            }
        }

        for (var key:String in statBoosts) {
            boost = statBoosts[key];
            var displayStat:String = boost.DisplayStat || boost.Stat;
            var amount:Number = boost.Amount;
            this.attributes += "  ";
            var space:String = boost.BoostType == "Static" ? " " : "% ";
            ...
            amount = Math.round(amount);
            this.attributes += TooltipHelper.wrapInFontTag(GetSign(amount) + amount + space, TooltipHelper.NO_DIFF_COLOR) + TooltipHelper.wrapInFontTag(displayStat, TooltipHelper.NO_DIFF_COLOR) + "\n";
        }
    }
    ...
    private function makeGemstoneMenu():void {
        if (this.itemData.GemstoneLimit < 1) {
            return;
        }

        this.specifications += TooltipHelper.wrapInFontTag("Shift + Click to edit gemstones.", "#FFFFFF") + "\n";
    }
    ...
    private function drawAttributes():void {
        ...
        this.attributesText = new SimpleText(14, 0xB3B3B3, false, WIDTH - 10, 0, false, Parameters.SPIEGEL);
    ...
    private function drawLine1():void {
        var color:uint = this.usableBy ? MoreColorUtil.adjustBrightness(this.rarityColor, -0.15) : 10965039;
        this.line1 = new Sprite();
        this.line1.x = 2;
        this.line1.y = this.descText.y + this.descText.height + 4;
        this.line1.graphics.lineStyle(1, color);
    ...
    private function makeSpecifications():void {
        ...
        else if (ItemConstants.isEquippable(this.itemData.SlotType)) {
    ...
    private function makeAttachableBy():void {
        if (!this.itemData.Gemstone || this.itemData.Gemstone.SlotTypes.length < 1) {
            return;
        }

        this.specifications += "Can be attached to: ";
        for each (var slotType:int in this.itemData.Gemstone.SlotTypes) {
            this.specifications += ItemConstants.itemTypeToName(slotType);
            if (!LastElement(slotType, this.itemData.Gemstone.SlotTypes)) {
                this.specifications += ", ";
            }
        }
        this.specifications += "\n";
    ...
    private function makeOrigin():void {
        if (!this.itemData.Gemstone || !this.itemData.Gemstone.Origin) {
            return;
        }

        var origin:String = this.itemData.Gemstone.Origin;
        this.specifications += "Origin: " + TooltipHelper.wrapInFontTag(origin, GetOriginColor(origin)) + "\n";
    }
    ...
    private function drawSpecifications():void {
        ...
        var sheet:StyleSheet = new StyleSheet();
        sheet.parseCSS(CSS_TEXT);
        this.specificationsText = new SimpleText(14, 0xB3B3B3, false, WIDTH - 10, 0, false, Parameters.SPIEGEL);
        ...
        this.positionSpecifications();
        addChild(this.specificationsText);
    }

    private function positionSpecifications():void {
    ...
    private function drawLine2():void {
        ...
        this.positionLine2();
        this.line2.graphics.lineStyle(1, color);
        this.line2.graphics.lineTo(WIDTH - 2, 0);
        this.line2.graphics.lineStyle();
        addChild(this.line2);
    }

    private function positionLine2():void {
        this.line2.x = 2;
        if (this.gemstoneUI)
            this.line2.y = this.line1.y + this.line1.height + ((Slot.HEIGHT + 4) * int(Math.ceil(this.itemData.GemstoneLimit / 3.0))) + 12;
        else if (this.attributesText)
    ...
    public function setGemstonesUI():void {
        if (!stage || this.itemData.GemstoneLimit < 1) {
            return;
        }

        if (this.gemstoneUI) {
            this.clearGemstoneUI();
            return;
        }

        this.setStayInStage(true);
        mouseEnabled = true;
        mouseChildren = true;
        this.followMouse_ = false;
        this.gemstoneUI = true;

        removeChild(this.attributesText);

        this.drawGemstoneSlots();
        this.positionLine2();
        this.positionSpecifications();
    ...
    public override function update() : void {
        //if icon is already null we prob shouldn't set it to something
        if(this.icon == null)
        {
            super.update();
            return;
        ...
        var tex:BitmapData = this.itemData.getRedrawnTexture(70, 0, true, getTimer());
        tex = BitmapUtil.cropToBitmapData(tex, 4, 4, tex.width - 8, tex.height - 8);
        if(this.icon.bitmapData == tex)
        {
            super.update();
            return;
        }

        this.icon.bitmapData = tex;
        super.update();
    }


    public function clearGemstoneUI():void {
        this.setStayInStage(false);
        if (!stage) {
            return;
        }
        ...
        mouseEnabled = true;
        mouseChildren = true;
        this.followMouse_ = true;
        this.gemstoneUI = false;

        addChild(this.attributesText);

        this.clearGemstoneSlots();
        this.positionLine2();
        this.positionSpecifications();
    ...
    private function drawGemstoneSlots():void {
        if (this.gemstoneSlots) {
            for (var i:int = 0; i < this.gemstoneSlots.length; i++) {
                addChild(this.gemstoneSlots[i]);
            }
            return;
        }

        var totalWidth:int = 3 * (Slot.WIDTH + 4); // We already know we have 3 slots per row
        this.gemstoneSlots = new Vector.<GemstoneSlot>();
        for (i = 0; i < this.itemData.GemstoneLimit; i++) {
            var slot:GemstoneSlot = new GemstoneSlot(i, this.itemData.SlotType);
            slot.x = (WIDTH - totalWidth) / 2 + (slot.width + 4) * int(i % 3);
            slot.y = this.line1.y + this.line1.height + 7 + (slot.height + 5) * int(i / 3);
            slot.addEventListener(ItemTileEvent.ITEM_MOVE, this.onGemstoneItemMove);
            slot.addEventListener(Event.CHANGE, this.onGemstoneChange); // An item was moved from inventory to this gemstone slot
            addChild(slot);
            this.gemstoneSlots.push(slot);
        }

        this.resetGemSlots();
    }

    private function resetGemSlots():void {
        if (!this.itemData.Gemstones || this.itemData.Gemstones.length < 1) {
            return;
        }
        for (var i:int = 0; i < this.gemstoneSlots.length; i++) {
            var slot:GemstoneSlot = this.gemstoneSlots[i];
            var item:ItemData = null;
            if (i < this.itemData.Gemstones.length) {
                item = new ItemData(this.itemData.Gemstones[i]);
            }
            slot.setItem(item, -1);
        }
    }

    private function clearGemstoneSlots():void {
        if (!this.gemstoneSlots) {
            return;
        }

        for (var i:int = 0; i < this.gemstoneSlots.length; i++) {
            removeChild(this.gemstoneSlots[i]);
        }
    }

    private function onGemstoneItemMove(e:ItemTileEvent):void { // Gemstone left the item
        var sourceSlot:Slot = e.slot;
        var targetTile:InteractiveItemTile = null;
        var target:* = DisplayHierarchy.getParentWithTypeArray(sourceSlot.getDropTarget(), Slot, InteractiveItemTile);
        if (target is InteractiveItemTile) {
            targetTile = target as InteractiveItemTile;
            if (targetTile.canHoldItem(sourceSlot.getItemId()) && sourceSlot.canHoldItem(targetTile.getItemId())) {
                this.swapSlotToTile(sourceSlot, targetTile);
            }
        } else if (target is Slot) {
            // Don't swap
        }
        sourceSlot.resetItemPosition();

        if (target is Slot || !(target is InteractiveItemTile)) { // Don't handle slot to slot movements
            return;
        }

        sourceSlot.setItem(null, -1);
        this.itemData.Gemstones.splice(sourceSlot.slotId, 1);
        this.resetGemSlots();

        GameServerConnection.instance.gemstoneRemove(this.invSlot, sourceSlot.slotId, targetTile.tileId);
    }

    private function onGemstoneChange(e:Event):void { // Gemstone enters the item
        var slot:GemstoneSlot = e.target as GemstoneSlot;
        if (slot.itemInvId == -1) {
            return;
        }

        if (slot.getItemId() == -1) {
            GameServerConnection.instance.gemstoneApply(this.invSlot, slot.slotId, slot.itemInvId);
        } else {
            GameServerConnection.instance.gemstoneSwap(this.invSlot, slot.slotId, slot.itemInvId);
        }
    }

    private function swapSlotToTile(sourceSlot:Slot, targetTile:InteractiveItemTile):void {
        var tempItem:ItemData = sourceSlot.getItemData();
        var tempItemType:int = sourceSlot.getItemId();
        var oldItem:ItemData = targetTile.getItemData();
        sourceSlot.setItem(oldItem, targetTile.tileId);
        targetTile.setItem(tempItem);
        targetTile.updateUseability(this.player);

        this.player.equipment_[targetTile.tileId] = tempItem;
        this.player.itemTypes[targetTile.tileId] = tempItemType;
    }
    ...
    private static function GetConditionEffectIndex(eff:int, effects:Vector.<ConditionEffectDesc>):int {
    ...
    private static function ApplyWisMod(value:Number, player:Player, offset:int = 1):Number {
        ...
        var wisdom:Number = Math.min(player.wisdom, MAX_WISMOD);
    ...
    private static function GetGemstoneBoost(boosts:Vector.<GemstoneBoost>, stat:String):GemstoneBoost {
        for each (var boost:GemstoneBoost in boosts)
            if (boost.Stat == stat)
                return boost;
        return null;
    }

    private static function GetOriginColor(origin:String):String {
        switch (origin) {
            default:
                return "#FFFFFF";
        }
