...
public class ObjectLibrary {
    ...
    public static function getTypeFromId(id:String) : int
    {
        return idToType_[id];
    }
    ...
    public static function getPropsFromType(objectType:int) : ObjectProperties
    {
        return propsLibrary_[objectType];
    }
    ...
    public static function getRedrawnTextureFromType(objectType:int, size:int, includeBottom:Boolean, useCaching:Boolean = true, scaleValue:int = 5, glowColor:uint = 0, scaleSize:Boolean = false, nonTransparent:Boolean = false):BitmapData {
        var textureData:TextureData = typeToTextureData_[objectType];
        ...
        if (nonTransparent){
            texture = TextureRedrawer.removeTransparentPixels(texture);
        }
    ...
    public static function isUsableByPlayer(objectType:int, player:Player):Boolean {
        ...
        if (slotType == ItemConstants.POTION_TYPE || slotType == ItemConstants.NO_ITEM) {
            return true;
        }
