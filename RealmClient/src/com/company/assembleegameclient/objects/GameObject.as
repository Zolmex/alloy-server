...
public class GameObject extends BasicObject {
    ...
    public static const HITBOX_RADIUS_SQR:Number = HITBOX_RADIUS * HITBOX_RADIUS;
    ...
    public function GameObject(objectXML:XML) {
        ...
        if (objectXML.hasOwnProperty("Defense")) {
            this.armor = int(objectXML.Defense);
        }
    ...
    public static function damageWithDefense(origDamage:int, targetDefense:int, armorPiercing:Boolean, targetCondition:Vector.<uint>):int {
    ...
    public function isStunImmune():Boolean {
        return (this.condition_[ConditionEffect.CE_FIRST_BATCH] & ConditionEffect.STUN_IMMUNE_BIT) != 0;
    }
    ...
    public function isInStance():Boolean {
        return (this.condition_[ConditionEffect.CE_SECOND_BATCH] & ConditionEffect.SHEATHE_STANCE_BIT) != 0;
    }
    ...
    public function onTickPos(x:Number, y:Number):void {
        ...
        if (this.props_.noInterpolation){
            this.onGoto(x, y, 0);
            return;
        }

        var magic:Number = 127 * (this.props_.interpolationAmount / 100.0);

        this.targetPosition_.x = x;
        this.targetPosition_.y = y;
        this.direction_.x = (this.targetPosition_.x - this.x_) / magic;
        this.direction_.y = (this.targetPosition_.y - this.y_) / magic;
    }
    ...
    public function damage(damageAmount:int, effects:Vector.<uint>, proj:Projectile):void {
        ...
        if (Parameters.data_.particles && hp - damageAmount > 0) {
            if (proj != null) {
                map_.addObj(new HitEffect(colors, this.size_, 10, proj.angle_, proj.Path.GetSpeed()), x_, y_);
            } else {
                map_.addObj(new ExplosionEffect(colors, this.size_, 10), x_, y_);
            }
        }
        if (damageAmount > 0) {
            pierced = this.isArmorBroken() || proj != null  && proj.armorPiercing_;
            this.damageStatus(damageAmount, pierced, proj != null ? proj.critDmg_ : 0, 1000);
        }
    }

    public function damageStatus(dmg:int, pierced:Boolean, critDmg:Number = 1, lifetime:int = 1000):void
    {
        var isCrit:Boolean = critDmg > 1;

        var text:String = "-" + dmg ;
        if (isCrit) text += " [" + roundToPrecision(critDmg, 2) + "x]";

        var color:uint = isCrit ? 0xffc200 : (pierced ? 0x9000FF : 0xFF0000);

        map_.mapOverlay_.addStatusText(new CharacterStatusText(this, text, color, lifetime))
    }
    ...
    protected function generateNameText(name:String):SimpleText {
        ...
        nameText.setFont(Parameters.SPIEGEL);
    ...
    public function getPortraitTexture() : BitmapData {
        return this.props_.portrait_ != null ? this.props_.portrait_.getTexture() : this.texture_;
    }
    ...
    public function getPortrait():BitmapData {
        ...
        var size:int = 0;
        if (this.portrait_ == null) {
            portraitTexture = getPortraitTexture();
    ...
    override public function draw(graphicsData:Vector.<IGraphicsData>, camera:Camera, time:int) : void
    {
        ...
        graphicsData.push(this.path_);
        graphicsData.push(GraphicsUtil.END_FILL);
        if(this.condition_[ConditionEffect.CE_FIRST_BATCH] || this.condition_[ConditionEffect.CE_SECOND_BATCH])
        {
