...
public class LegendFactory {
    ...
    private static const MAX_LEGENDS:int = 20;
    ...
    private function makeLegendsFromList(list:XMLList, isFocus:Boolean):void {
        ...
        this.legends.sort(legendsComparer);
        if (this.legends.length > MAX_LEGENDS){
            this.legends.splice(20, this.legends.length - 20);
        }
    }

    private static function legendsComparer(legendA:Legend, legendB:Legend):int {
        if (legendA.totalFame > legendB.totalFame){
            return -1; // A goes before B
        }
        else if (legendA.totalFame < legendB.totalFame){
    ...
    public function makeLegend(xml:XML):Legend {
        ...
        var itemTypes:Vector.<int> = ConversionUtil.toIntVector(xml.Equipment);
        legend.equipment = new Vector.<ItemData>();
        var stream:ByteArray = ItemData.GetStream(xml.ItemDatas);
        for (var i:int = 0; i < itemTypes.length; i++){
            var itemType:int = itemTypes[i];
            if (itemType == -1) {
                legend.equipment.push(null);
                continue;
            }

            var itemData:ItemData = new ItemData(itemType);
            itemData.ParseData(stream);
            legend.equipment.push(itemData);
        }
        return legend;
    }
}
