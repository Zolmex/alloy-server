...
namespace Common.Control
{
    public static class ServerControl
    {
        ...
        public static void Connect(MemberType type, string memberName, ServerInfo serverInfo = null)
        {
            ...
            Host = new ServerMember() { Type = type, Name = memberName, InstanceID = _instanceID, ServerInfo = serverInfo };
            ...
            while (true)
            {
                ...
                foreach (var kvp in _serversTimeout)
                {
                    if (kvp.Value >= TIMEOUT_MS)
                    {
                        ...
                        if (host != null)
                            MemberLeft.Invoke(null, new ControlMessage<ServerMember>() { InstanceID = host.InstanceID, TargetID = null, Content = host });
                        remove.Add(kvp.Key);
                    }
                }
        ...
        {
            var message = new ControlMessage<T>() { InstanceID = senderId, TargetID = targetId, Content = content };
