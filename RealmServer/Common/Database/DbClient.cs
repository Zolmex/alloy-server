...
public static class DbClient
{
    ...
    public static async Task<RegisterStatus> Register(string username, string password, string ip)
    {
        ...
        var ack = (RegisterMessageAck)await _con.SendAsync(
            new RegisterMessage
            {
                Username = username,
                IPAddress = ip,
    ...
    public static async Task<VerifyMessageAck> VerifyAccount(string username, string password)
    {
        var ack = (VerifyMessageAck)await _con.SendAsync(
            new VerifyMessage
            {
                Username = username,
                Password = password
