...
public class EventController : Controller
{
    ...
    public IActionResult SubscribeEventSpawn([FromBody] EventSpawnSubscriptionRequest request)
    {
        ...
        if (eventSpawnSubscriptions[SubscribableEvent.All].Where(x => x.CallbackUrl == request.CallbackUrl).Any())
        {
    ...
    public IActionResult SubscribeEventDeath([FromBody] EventDeathSubscriptionRequest request)
    {
        ...
        if (eventDeathSubscriptions[SubscribableEvent.All].Where(x => x.CallbackUrl == request.CallbackUrl).Any())
        {
            return Conflict("Already subscribed to all events.");
        }
    ...
    private static readonly Dictionary<SubscribableEvent, List<EventLifecycleSubscription>> eventSpawnSubscriptions = new()
    {
    ...
    private static readonly Dictionary<SubscribableEvent, List<EventLifecycleSubscription>> eventDeathSubscriptions = new()
    {
        { SubscribableEvent.All, new List<EventLifecycleSubscription>() },
        { SubscribableEvent.Avatar, new List<EventLifecycleSubscription>() },
        { SubscribableEvent.LordoftheLostLands, new List<EventLifecycleSubscription>() },
