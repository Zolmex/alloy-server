...
{
    private static readonly Dictionary<LootDropRarity, List<LootDropSubscription>> lootDropSubscriptions = new() { { LootDropRarity.All, new List<LootDropSubscription>() }, { LootDropRarity.Legendary, new List<LootDropSubscription>() }, { LootDropRarity.Primal, new List<LootDropSubscription>() } };
    ...
    public IActionResult SubscribeLootDrop([FromBody] LootDropSubscriptionRequest request)
    {
        ...
        if (lootDropSubscriptions[LootDropRarity.All].Where(x => x.CallbackUrl == request.CallbackUrl).Any())
        {
